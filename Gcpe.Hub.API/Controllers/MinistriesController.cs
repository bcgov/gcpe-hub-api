    ...
    public class MinistriesController : BaseController
    {
        ...
        public IActionResult GetAllMinistries()
        {
            try
            {
                ...
                return res ?? Ok(mapper.Map<List<Models.Ministry>>(dbMinistries.Where(m => (m.IsActive && !m.DisplayName.StartsWith("Joint Information Centre"))).OrderBy(m => m.SortOrder).ThenBy(m => m.DisplayName).ToList()));
            }
            catch (Exception ex)
            {
